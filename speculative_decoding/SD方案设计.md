...
### 13.7 哪些改动是真正有数据收益的，哪些主要是铺路

把整条版本演进线压缩成一句话：

- **真正把数据明显拉起来的，是 V4.2 和 V5.x 的 batch 化**
- **V0 ~ V4.0 更多是在把 speculative decoding 的控制流、KV 生命周期和 verify 语义做正确**

如果按“数据贡献”而不是“工程必要性”来排，大致可以分三档。

#### 第一档：直接拉高数据的核心改动

1. **V4.2：draft CUDA Graph + graph-friendly `store_kvcache`**
   - 这是单请求路线里**最关键**的一步
   - `run_test.py` 的提升几乎主要都来自这里
   - 从数据上看，单请求 speedup 基本就是在这一步第一次稳定跨过 `1.0x`

2. **V5.0 / V5.1：batch 化**
   - 这是多请求 serving 路线里**最大的收益来源**
   - V5.0 先证明“同步 batch=2 就已经值”
   - V5.1 再证明“任意 batch + 异步 phase”能把吞吐继续抬到 `3x~4x+`
...
1. **V4.1：消灭 Case C dummy decode**
   - 有明确收益，但量级远小于 V4.2
   - 更像是“把一条明显不必要的 draft 前向削掉”
   - 这类优化会让控制流更干净，也会让后面的 Graph/batch 更容易成立

#### 第三档：主要是铺路，不是直接靠它们把 tok/s 拉起来

下面这些版本都非常重要，但它们的价值更多体现在“没有它们，后面的大收益根本做不出来”：

- **V0 / V1**：把 speculative decoding 的最小闭环和 verify 接口打通
- **V2**：把 base 侧最重的 replay / adopt 逻辑理顺
- **V3**：让增量状态复用和 partial block 处理成立
- **V4.0**：把 fused q0 + verify 这条主路径做出来

这些版本的共同特点是：

- 对正确性至关重要
- 对后续版本是必要前提
- 但如果只盯着最终 benchmark，它们往往不是“最显眼的那次跃迁”

所以从现在的结果回头看，整条路线可以这么理解：

1. **V0 ~ V4.0：先把路修通**
2. **V4.1：把路修平**
3. **V4.2：第一次真正把单请求数据拉起来**
4. **V5.0 / V5.1：把 batch 这条主收益路线验证出来**

这也是为什么现在再看 `SD方案设计`，最值得记住的不是“每一版都涨了多少”，而是：

- **V4.2 证明单请求 SD 终于值得做**
- **V5.1 证明 batch 才是吞吐真正爆发的地方**

### 13.8 阶段性结论

如果只问一个问题：

> **`nano-vllm` 里“利用 draft model 做加速”这条工程路线，在当前 `Qwen3-4B + Qwen3-0.6B` 这套组合上是不是差不多做完了？**

当前阶段的判断可以很明确：

**从“纯工程把这条路线做通、做出明显收益”这个角度看，基本已经做完了。**

原因有三层。

#### 第一层：这条路线已经被验证为“真的能加速”

现在手上的数据已经同时证明了两件事：

1. **单请求路线成立**
   - V4.2 之后，`run_test.py` 里的单请求 speedup 已经稳定在 `1.2x~1.5x`
   - 这说明 draft model 不再只是“理论上可能有用”，而是在当前仓库里已经是**可观测、可复现的真实加速**

2. **batch 路线成立**
   - V5.0 / V5.1 之后，`run_batch_test.py` 已经把总吞吐拉到 `3x~4.3x`
   - 这说明 draft model 不只是能改善单请求 latency，更能显著提高多请求 serving throughput

也就是说，“draft model 能不能在 `nano-vllm` 里真正带来收益”这件事，现在已经不是 open question 了。

#### 第二层：还没完全收尾，但剩下的更多是“系统化”，不是“再找一个大加速点”

当前还没完成、但仍然重要的工程工作，主要是：

- **V5.2：和原生 `Scheduler` 融合**
- request lifecycle / phase 元数据内生化
- KV 生命周期、抢占、recompute、prefix restore 与 batch SD 对齐

这些事情会让系统：

- 更完整
- 更稳定
- 更像 inference engine 的内生能力

但它们的性质更接近**系统收尾 / 架构收敛**，而不太像 V4.2 或 V5.1 这种“还能再凭一轮纯工程把数据抬一个大台阶”的工作。
...
#### 第三层：如果还想显著更快，后面的主战场会越来越偏模型

继续显著提高 speedup，最终还是看两个量：

- **`α`（acceptance rate）**
- **`c = T_draft / T_base`（draft 相对 base 的成本比）**

而纯工程对这两个量的提升空间已经不大。后面如果还想要明显更大的收益，更可能来自：

1. **更好的 draft model**
   - 蒸馏
   - EAGLE / Medusa / MTP 一类路线
   - 目标是把 `α` 往上推

2. **更合适的 base / draft 组合**
   - 通过更大的 base 或更匹配的 draft 改善 `c` 和 `α`

3. **训练出来的多 token proposal 能力**
   - 这已经不是仓库内小修小补，而是模型路线本身的升级

所以，如果问题是“后面真正还有大收益空间的部分是不是更偏训练和模型设计”，答案基本是：

**是。**

#### 最后一句收束判断

当前最准确的结论不是“这条线彻底结束了”，而是：

- **靠纯工程把当前 draft model 这条加速路线做起来，这件事已经差不多做到头了**
- **后面剩下的大工程，是把它做完整（V5.2）**
- **后面如果还想拿明显更大的收益，主变量会越来越偏模型和训练，而不是继续做仓库内 micro-opt**

所以从项目阶段上看，现在已经可以把 `nano-vllm` 的 speculative decoding 视为进入了一个新阶段：

- 前半段是“把 draft model 加速这件事做成立”
- 后半段要么是“把系统融完整”，要么就是“去模型侧找下一次跃迁”
